...
function sendSMS($idTrans)
{
  ...
  curl_setopt_array($curl, array(
    CURLOPT_URL => 'https://mscloud.darielabreu.com/api/v1/template/pdf',
    ...
    CURLOPT_POSTFIELDS => '{
        ...
        "store_name": "' . $laAgencia["vendedor"] . '",
        "client_phone": "' . $celCliente . '",
        "police_number": "' . $poliza . '",
        "date_init": "' . FechaListPDFn($row['fecha_inicio']) . '",
        "date_end": "' . FechaListPDFin($row['fecha_fin']) . '",
        "template_name": "enviar_seguro",
        "poliza_id": "' . $row["id"] . '"
}',
